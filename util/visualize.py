...
def create_plot(mp):
    ...
    pv_gen = np.array([value(mp.blocks[i].process.fs.elec_generation) for i in range(n)])
    pv_curtail = np.array([value(mp.blocks[i].process.fs.curtailment) for i in range(n)])
    electric_price = np.array([value(mp.blocks[i].process.fs.electricity_price) for i in range(n)])
    ro_demand = np.array([value(mp.blocks[i].process.fs.RO.power_demand) for i in range(n)])
    grid_cost = ([value(mp.blocks[i].process.grid_cost)/
            value(pyunits.convert(6000 * pyunits.m**3 / pyunits.day, to_units=pyunits.m**3 / pyunits.hour)) for i in range(n) ])
    lcow = ([0.4+((value(mp.annualized_capital_cost) /365 / 24) + value(mp.blocks[i].process.grid_cost)) /
            value(pyunits.convert(6000 * pyunits.m**3 / pyunits.day, to_units=pyunits.m**3 / pyunits.hour)) for i in range(n)])
    ...
    ax.stackplot(hour, df["PV to RO"],  df["Battery to RO"], df["Grid to RO"], baseline='zero', colors=['#1f77b4','#ff7f0e','#d62728'], labels=labels, alpha=1, ec='white')
    ax.plot(hour, df["PV to Battery"], label="PV to Battery", color='#2ca02c', linewidth=2)
    ax.fill_between(hour, df["PV to Battery"], color='#2ca02c', hatch='////', edgecolor="#515251", linewidth=2, alpha=0.5)
    ...
    line1 = ax4.plot(hour, electric_price, dashes=[4, 4], lw=2.5, color="white", label='Grid Price')  
    ...
    ab = ax5.annotate(f"LCOW=${mp.LCOW():1.2f}", (0.01, 0.9), xycoords='axes fraction', 
                        fontsize=16, color="k",
                        bbox=dict(boxstyle="square",
                        fc="white", ec="k", lw=1))
    
    ac = ax5.annotate(f"Battery Size={value(mp.blocks[0].process.fs.battery.nameplate_energy):1.0f} kWh", (0.82, 0.9), xycoords='axes fraction', 
                        fontsize=16, color="k",
                        bbox=dict(boxstyle="square",
                        fc="white", ec="k", lw=1))
    ...
    plt.figlegend(loc='upper left', fontsize=16, frameon = True, 
                  bbox_to_anchor=(0.12, 0.51, 1.0, 0.51), ncols=6, 
                  facecolor='#d4d2d2', edgecolor=None, framealpha=1)
